...
function customise(params)
   ...
   -- CPU with two cores; two gauges with same x,y and different radius
   --                     defaults can be used for angles, size and max/min
   table.insert(params, { name = 'cpu', arg = 'cpu1',
                          x = 85, y = 190,
                       })
   table.insert(params, { name = 'cpu', arg = 'cpu2',
                          x = 85, y = 190, radius = 18,
			  indicator_offset = -9,
                       })
   ...
   -- Temperature; two gauges for CPU and HDD, using a range of 30-110C with
   --              captions indicating which is which. Assume that 90C is
   --              critical so adjust to 0.75
   table.insert(params, { name = 'hwmon', arg = 'temp 2', caption = 'CPU',
			  x = 85, y = 380,
			  min_value = 30, max_value = 110, critical_trip = 0.75,
		       })
   table.insert(params, { name = 'hwmon', arg = '1 temp 3', caption = 'HDD',
			  x = 85, y = 380, radius = 18,
			  indicator_offset = -9,
			  min_value = 30, max_value = 110, critical_trip = 0.75,
		       })
